...
| `pr-revised.sh <slug> <pr_url> <resume_sig> <mode> <review_limit> <project>` | `event:pr-revised:<url>` | Auto-review: increments counter, spawns reviewer if ≤ limit else escalates (pr-monitor already running); Standard: forwards as pr-submitted |
| `pr-ready-final.sh <slug> <pr_url>` | `event:pr-ready-final:<url>` | Forwards as `event:pr-ready` to Spokesman (pr-monitor already running from first pr-ready) |
